function [mu,isig] = spm_affine_priors_SM(typ)
...
case 'mni', % For registering with MNI templates...
    mu   = [1e-4 1e-4 0.01 1e-2 1e-4 1e-4 0.1 0.001 0.1 0.1 0.01 0.1]';
%    mu   = [1e-1 1e-1 0.001 1e-2 1e-1 1e-1 1e-4 1e-2 1e-4 1e-4 1e-4 1e-4]';
    ...
    isig = 1e3*diag(mu); 
    isig(7,:) = isig(7,:)-1e2*cat(2,[0.0667 0.0039 0.0008 0.0333 0.0071 0.1071],[0.0667 0.0039 0.0008 0.0333 0.0071 0.1071]);
    isig(7:12,7:12) = isig1+isig(7:12,7:12);
...
case 'imni', % For registering with MNI templates...
    ...
    isig = 1e4 * [
        0.0902   -0.0345   -0.0106   -0.0025   -0.0005   -0.0163
       -0.0345    0.7901    0.3883    0.0041   -0.0103   -0.0116
       -0.0106    0.3883    2.2599    0.0113    0.0396   -0.0060+3
       -0.0025    0.0041    0.0113    0.0925    0.0471   -0.0440
       -0.0005   -0.0103    0.0396    0.0471    0.2964   -0.0062
       -0.0163   -0.0116   -0.0060   -0.0440   -0.0062    0.1144];
...
case 'rigid', % Constrained to be almost rigid...
   ...
   isig(1,1) = 1e3; % x-scaling
   isig(4,4) = 1e3; % y-scaling
   isig(6,6) = 1e3; % y-scaling
