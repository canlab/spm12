function varargout = tbxdti_acid2_run_favbs_norm(cmd, varargin)
...
% hack: does not allow to use spinal cord modus
job{1}.SCDTI = 0; 
        ...
        case 'run'
            ...
            while(citer<=job{1}.norm.niter)
                ...
                for isteps=1:numel(job{1}.norm.steps),
                    ...
                    out{iter_all} = cfg_acid2_run_favbs_create_template('run',job{iter_all},char(fieldnames(job{1}.norm.steps{isteps})));
                    if(iter_all==1)
                        ...
                        if(iter_all==2)
                            for k = 1:numel(out{1}.matnames)
                                ...
                                inputs{(k-1)*4 + 3} = n;
                                inputs{(k-1)*4 + 4} = [ACID_my_select(deblank(job{1}.PFA(k))) ACID_my_select(deblank(job{1}.Pmean(k))) ACID_my_select(deblank(job{1}.Pb0(k)))];
                                jobComb{k} = 'batch_combine_snMATs_job.m';
                            end
                        else % if iter_all > 2
                            for k = 1:numel(out{iter_all}.matnames)
                                ...
                                inputs{(k-1)*4 + 3} = n;
                                inputs{(k-1)*4 + 4} = [ACID_my_select(deblank(job{1}.PFA(k))) ACID_my_select(deblank(job{1}.PFA(k))) ACID_my_select(deblank(job{1}.PFA(k)))];
                                jobComb{k} = 'batch_combine_deffield_snMAT_job.m';
                            end
                        end
                        ijobComb=iter_all-1;
                        % output "jobComb" unterscheidet sich vom output
                        % "cfg_acid2_run_favbs_create_template", daher ist eine
                        % neue variable "outjobComb" notwendig
                        cdir = pwd;
                        cd(p)
                        ver = spm('Version');
                        if(strfind(ver,'SPM12')==true)
                            [outjobComb{ijobComb} hjob]  = spm_jobman('serial',jobComb, '',inputs{:});
                        elseif(strfind(ver,'SPM8')==true)
                        ...
                        job{iter_all+1}.citer      = 1;
                        job{iter_all+1}.SCDTI      = job{1}.SCDTI;
                        job{iter_all+1}.cutoff     = job{1}.cutoff-round(0.2*job{1}.cutoff);
            ...
            if(isfield(job{1}.norm.templ,'CT'))% case 1: Customized Template is symmetrical
                if(isfield(job{1}.norm.templ.CT,'SCT'))
                    if(job{1}.norm.templ.CT.SCT==1)
                        if(exist('iter_all'))
                            out{iter_all+1} = cfg_acid2_run_favbs_create_template('run',job{iter_all+1},'LFA');
                        else
                            out{1} = cfg_acid2_run_favbs_create_template('run',job{1},'LFA');
                        end
                    end
                end
