function varargout = tbx_acid_run_favbs_norm(cmd, varargin)
        ...
        case 'run'
            ...
            while(citer<=job{1}.norm.niter)
                ...
                for isteps=1:numel(job{1}.norm.steps),
                    ...
                    out{iter_all} = cfg_run_favbs_acid_create_template('run',job{iter_all},char(fieldnames(job{1}.norm.steps{isteps})));
                    if(iter_all==1)
                        ...
                        % "cfg_run_favbs_acid_create_template", daher ist eine
            ...
            if(isfield(job{1}.norm.templ,'CT'))% case 1: Customized Template is symmetrical
                if(isfield(job{1}.norm.templ.CT,'SCT'))
                    if(job{1}.norm.templ.CT.SCT==1)
                        if(exist('iter_all'))
                            out{iter_all+1} = cfg_run_favbs_acid_create_template('run',job{iter_all+1},'LFA');
                        else
                            out{1} = cfg_run_favbs_acid_create_template('run',job{1},'LFA');
                        end
                    end
                end
            else  % case 2: External LFA Template is not empty (Multi-Contrast)
                if(isfield(job{1}.norm.templ.ET,'SC'))
                    if(isfield(job{1}.norm.templ.ET.SC,'PGLFA'))
