function varargout = cfg_run_favbs_acid_create_template(cmd, varargin)
      ...
      % Begin: do the job   
            ...
            if(strcmp('CT',fieldnames(job.norm.templ))) % Customized Template (CT) Generation
                if(strcmp('SCT',fieldnames(job.norm.templ.CT))) % Symmetrisized CT
                    out = acid_FAVBS_do_the_job(rVb0, rVFA, Vmean, FA_on,iter,MSKperc,1);
                elseif(strcmp('NCT',fieldnames(job.norm.templ.CT))) % Normal CT
                    out = acid_FAVBS_do_the_job(rVb0, rVFA, Vmean, FA_on,iter,MSKperc,0);
                end
            elseif(strcmp('ET',fieldnames(job.norm.templ))) % External Template (ET) 
                ...
                out = acid_FAVBS_do_the_job(rVb0, rVFA, Vmean, FA_on, iter,MSKperc, VG);
            end
