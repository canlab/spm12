function params = spm_normalise_SM(VG,VF,matname,VWG,VWF,flags)
...
if(flags.cutoff >= 20)
    ...
    [M,scal]  = spm_affreg_SM(VG1, VF1, aflags, M);

    fprintf('Fine Affine Registration..\n');
    aflags.WG  = VWG;
    aflags.WF  = VWF;
    aflags.sep = aflags.sep/2;
    [M,scal]   = spm_affreg_SM(VG1, VF1, aflags, M,scal);
end
...
k    = max(round((VG(1).dim(1:3))./cutoff),[1 1 1]);
