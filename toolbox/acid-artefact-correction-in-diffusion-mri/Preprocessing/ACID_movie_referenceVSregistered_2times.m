function ACID_movie_referenceVSregistered_2times(PARef,PAsource,PAsource1,iARef,iASource,zpos,localtime,dummy_movie,res)
% make movie
...
if(~exist('PAsource1','var'))
    PAsource1   = char(cfg_getfile(Inf,'IMAGE',['Get Registered image'],'','','.*'));
end

Asource1 = zeros([dm size(PAsource1,1)]);
V = spm_vol(PAsource1);
for i=1:size(V,1)
    Asource1(:,:,:,i)    = ACID_read_vols(V(i),VG,res);
end
...
if(~dummy_movie)
    ...
    for i=1:size(Asource,4)
        subplot(1,3,1);imagesc(rot90(ARef(:,:,zpos)),[iARef(1) iARef(2)]); title('Reference image'); axis off
        subplot(1,3,2);imagesc(rot90(Asource(:,:,zpos,i)),[iASource(1) iASource(2)]); title(['Original image' num2str(i)]); axis off
        subplot(1,3,3);imagesc(rot90(Asource1(:,:,zpos,i)),[iASource(1) iASource(2)]); title(['Source image' num2str(i)]); axis off
        pause(localtime)
    end
    close(f1);
else
