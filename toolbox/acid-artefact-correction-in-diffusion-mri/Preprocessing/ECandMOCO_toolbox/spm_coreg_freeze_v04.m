function x = spm_coreg_freeze_v04(varargin)
...
if ~isfield(VG, 'uint8'),
    VG.uint8 = loaduint8_SM1(VG,flags.Abias);
...
for k=1:numel(VF),
    ...
    if(~isempty(find(spm_read_vols(VFk))>0))
        if ~isfield(VFk, 'uint8'),
            VFk.uint8 = loaduint8_SM1(VFk,flags.Abias);
...
%_______________________________________________________________________
function udat = loaduint8_SM1(V,Abias)
...
switch phase_encoding
    case 1
        ...
        V.dim = V.dim([2 1 3]);
        Abias = permute(Abias,[2 1 3]);
    case 2        
        % nothing changes
    case 3
        V.mat = V.mat([1 3 2 4],:); % (:,[2 1 3 4])
        V.dim = V.dim([1 3 2]);
        Abias = permute(Abias,[1 3 2]);
end
% Load data from file indicated by V into an array of unsigned bytes.
if size(V.pinfo,2)==1 && V.pinfo(1) == 2,
    mx = 255*V.pinfo(1) + V.pinfo(2);
    ...
    for p=1:V.dim(3),
        img = spm_slice_vol(V,spm_matrix_mod([0 0 p]),V.dim(1:2),1).*min(Abias(:,:,p),1e1);
        mx  = max([max(img(:))+paccuracy(V,p) mx]);
        mn  = min([min(img(:)) mn]);
        if(dummy_movie==true)
            figure(f1);subplot(1,2,1);imagesc(img);
            
            imgtmp = spm_slice_vol(V,spm_matrix_mod([0 0 p]),V.dim(1:2),1);
            mx  = max([max(imgtmp(:))+paccuracy(V,p) mx]);
            mn  = min([min(imgtmp(:)) mn]);
            subplot(1,2,2);imagesc(imgtmp);
            drawnow
            clear imgtmp
        end
        spm_progress_bar('Set',p);
