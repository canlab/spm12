function [dummy_4D,V4D] = POAS_v04( DiffVec, bvalues, kstar, kappa0, lambda, sigma, ncoils, PInd,PMSK, dummy_4D)
...
%    lambda      = 12; % JP  check if this is a good value, may need to adjust it
    ...
    if(~isempty(PInd))
        ...
        if(size(PInd,1)>=2)
            Vtmp1=spm_vol(PInd(1,:));
            Vtmp2=spm_vol(PInd(2,:));
...
% input to adsmse3ms coincides with what we observe in R
    ...
    if(~dummy_4D)
        ...
        zth = cat(4,zth0,zth);
        V4D = poas_write_image_4d(zth,V,V(MSK_b0(1)),prefix);
    end
