function POAS_v02( DiffVec, bvalues, kstar, kappa0, lambda, sigma, ncoils, PInd)
...
% input to adsmse3ms coincides with what we observe in R
    ...
    for i=1:numel(MSK_b)
        ...
        my_write_vol_nii(zth(:,:,:,i),V(MSK_b0(1)),prefix,ending);
    end
    prefix = 'poas_b0_';
    % b=0 image
    ending='-000';
    my_write_vol_nii(zth0,V(MSK_b0(1)),prefix,ending);
