...
#define THRESHOLD 5
#define CONS 1.1 //ohne skalierung ; der faktor zwei ist dazu da, die fehlende korrektur durch cADC auszugleichen. 
struct point
{
    int xa;
    int n;
...
void GVtoTens (double **TV, double *GV, int NG) //macht aus einem #ofGradx3 Vektor eine 6x#ofGrad Matrix (desine-matrix?!? - geometrische Matrix)
 {
 ...
 void TVtoXM (double **XM, double **TV, double *SIG, int NG) // macht aus der geometrischen Matrix (6 x #ofGrad) eine symetrische (#ofGrad x #ofGrad): XM
 {
    int i, j, k;
    double sum;
 ...
 void NteSPALTE_XM (double **VEC, double **XM, double N_XM) // macht aus der geometrischen Matrix (6 x #ofGrad) eine symetrische (#ofGrad x #ofGrad)
 {
    int i, NX;
    NX = N_XM;
    for (i=1; i <= ZEILEN; i++)
...
int max1(float *egv)
    {  
        if (egv[1]*egv[1] > egv[2]*egv[2])
        {
            if (egv[1]*egv[1] > egv[3]*egv[3]) return 1; 
            else return 3;
        } 
		else 
		{
		    if (egv[2]*egv[2] > egv[3]*egv[3]) return 2;
            else return 3;
        }
	}
...
//    DM      = mxGetPr (prhs[2]);
//    R       = mxGetPr (prhs[3]);
//    cADC    = mxGetPr (prhs[4]); // input: whole-brain averaged ADC for each diffusion-weighted image 
//    iMEC    = mxGetPr (prhs[5]); // input: voxel-wise EC matrix
     
   
    plhs[0] = mxCreateNumericArray (ndims, out_dims, mxDOUBLE_CLASS, mxREAL); // residual error
//    nSIGMA   = mxGetPr (plhs[0]);
//    plhs[1] = mxCreateNumericArray (ndims, out_dims, mxDOUBLE_CLASS, mxREAL); // residual error
    FA      = mxGetPr (plhs[0]);

//    plhs[2]         = mxCreateNumericArray (ndims, out_dims, mxDOUBLE_CLASS, mxREAL); // residual error
//    SIGMA_bck       = mxGetPr (plhs[2]);
//    plhs[3]         = mxCreateNumericArray (ndims, out_dims, mxDOUBLE_CLASS, mxREAL); // residual error
//    nSIGMA_bck      = mxGetPr (plhs[3]); 
//    plhs[4] = mxCreateNumericArray (ndims, out2_dims, mxDOUBLE_CLASS, mxREAL); // ADC maps
//    oADC    = mxGetPr (plhs[4]);
    plhs[1] = mxCreateNumericArray (ndims, out3_dims, mxDOUBLE_CLASS, mxREAL); // Eigenvectors
    EV    = mxGetPr (plhs[1]);
    plhs[2] = mxCreateNumericArray (ndims, out3_dims, mxDOUBLE_CLASS, mxREAL); // Eigenvalues
    EW    = mxGetPr (plhs[2]);
//    plhs[3] = mxCreateNumericArray (ndims, out4_dims, mxDOUBLE_CLASS, mxREAL); // The MEC tensor
//    MEC   = mxGetPr (plhs[7]);
    plhs[3] = mxCreateNumericArray (ndims, out_dims, mxDOUBLE_CLASS, mxREAL); // Map with outliers
