...
void VoxelPos ( double *FA, double *iVOL, const int *dims, double *EV, double *EW, double *outlV)
{
    ...
    struct point i;
    int j, in_vox, num_rot,  eig1, eig2;
    float av, dM[2];
    ...
    for (i.xa = 0; i.xa < dims[0]; i.xa++)
    {
        ...
        if((dM[0]+dM[1]+dM[2]) >0) 
        {
            ...
            av          = (DD[1][1] + DD[2][2])/2;
            FA[pos]     = 1000.0 * sqrt( 2 * ((eigenvalues[1]-av) * (eigenvalues[1]-av) + (eigenvalues[2]-av) * (eigenvalues[2]-av))) / sqrt( 2.0 * ( eigenvalues[1] * eigenvalues[1] + eigenvalues[2] * eigenvalues[2]) );
