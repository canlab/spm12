function [Asymvol_robust,w0vol,sigma] = robust_estimate_Zwiers2010_MPMs(DM,Adti,Asymvol,AMSK)
% slice-wise robust fitting
% S. Mohammadi 12/06/2012
    ...
    vsz = size(Asymvol);
    w0vol = zeros(vsz);
    Asymvol_robust = Asymvol;
 
    
    % make out of brain (aob) and brain (brain) mask
    MSK_brain   = make_MSK(exp(Adti),perc);
    AMSK_aob    = ones(vsz(1:3));
    AMSK_brain  = zeros(vsz(1:3));
    AMSK_aob(MSK_brain) = 0;
    AMSK_brain(MSK_brain)=1;
        
    % define sigma
    sigma = zeros(1,size(Adti,4));
    for i = 1:size(Adti,4)
        tmp = Adti(:,:,:,i);
    ...
    for zpos = 1:vsz(3)
        AMSKslice   = AMSK(:,:,zpos);
        lSWslice    = zeros([numel(find(AMSKslice>0)) size(Adti,4)]);
        Asym0slice  = zeros([numel(find(AMSKslice>0)) size(Asymvol,4)]);
        ...
        while (rms_dDT > thr_DTvar && iter < 10)
            ...
            if(iter==1)
                [Asym_tmp0,w0]   = roustestimate_DT(Asym0slice,AMSKslice,DM,lSWslice,thr_w0,sigma,MSKslice2);
            else
                Asym_tmp0    = Asym_tmp;
            end
            [Asym_tmp,w0] = roustestimate_DT(Asym_tmp0,AMSKslice,DM,lSWslice,thr_w0,sigma,MSKslice2);
...
% ------------------------------------------------------------------------
function [Asym_w,w0] = roustestimate_DT(Asym0,AMSK,DM,lSWvol,thr_w0,sigma,MSKslice2)
...
for i=1:size(lSWvol,1)
    Vcond(i)=cond(DM(MSK_b,2)'*diag(w0(i,MSK_b))*DM(MSK_b,2));
