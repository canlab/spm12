function [Asym_robust,w0robust,sigma] = robust_estimate_Zwiers2010_slicew_26072012(DiffVecORIG,lSWvol,Asym0,AMSK,bvalue)
...
%     prior   = exp(DM'*mDM);
    ...
    for zpos = 1:vsz(3)
        ...
        while (rms_dDT > thr_DTvar && iter < 10)
            ...
            if(iter==1)
                [Asym_tmp0,w0]   = roustestimate_DT(Asym0slice,AMSKslice,DM,lSWslice,thr_w0,bvalue,sigma,MSKslice2);
            else
                Asym_tmp0    = Asym_tmp;
            end
            [Asym_tmp,w0] = roustestimate_DT(Asym_tmp0,AMSKslice,DM,lSWslice,thr_w0,bvalue,sigma,MSKslice2);
...
% ------------------------------------------------------------------------
function [Asym_w,w0] = roustestimate_DT(Asym0,AMSK,DM,lSWvol,thr_w0,bvalue,sigma,MSKslice2)
