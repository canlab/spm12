function [Asym_w,wout] = roustestimate_DT_prod(Asym0,AMSK,DM,lSWvol,reg,bvalue,sigma,MSKslice2,prefactor,maxk,mb,zpos,F1,iter,dummy_DKI_robust,V)
...
if(numel(lSWvol)==0)
    ...
    % determine
    [bC,bia,bic] = unique(bvalue(1,bvalue>=mb));  
...
%             tmp(:,MSK_nb)    = (exp(bsxfun(@rdivide, mean(lSthr(:,MSK_nb),2),bsxfun(@times,ones(size(lSthr(:,MSK_nb))),sigma(MSK_nb))))).^2; 
            tmp     = (exp(bsxfun(@rdivide,lSthr,sigma))).^2; 
        else
    ...
    if(mresplane~=0 && abs(mresplane)<Inf)
        tmptmp  = exp(-2*(bsxfun(@rdivide,prefactor/3*resplane(mresplane>0,:),mresplane(mresplane>0))).^2);
    ...
    % check condition
%    if(iter==1 || ~isempty(lastwarn))
%        w0 = check_cond(DM,MSK_b,w0);
%    end
    mtmp = mean(tmp,1);
    for i=1:numel(bC)
        MSK_b_tmp = find(bic==i);
        tmpw0_b = max(w0(:,MSK_b_tmp),[],2); w0(:,MSK_b_tmp)      = bsxfun(@rdivide, w0(:,MSK_b_tmp),tmpw0_b);  
        tmpw0_b = max(wout(:,MSK_b_tmp),[],2); wout(:,MSK_b_tmp)      = bsxfun(@rdivide, wout(:,MSK_b_tmp),tmpw0_b);  
    end
    ...
    YY      = Aw0*yyw(MSK_nz);
    YYY     = X(:,MSK_nz)*YY;
    xxx     = (speye(numel(MSK_nz))*X(:,MSK_nz)');
    xxx     = X(:,MSK_nz)*xxx;
    clear XX;
    clear YY;
    % Atmp_w  = XXX\YYY; %  C = mldivide(A,B)
    Atmp_w  = (XXX+reg*xxx)\YYY; % hack SM
