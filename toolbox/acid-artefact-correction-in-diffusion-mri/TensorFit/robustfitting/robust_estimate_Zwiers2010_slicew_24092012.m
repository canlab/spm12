function [Asym_robust,w0robust,sigma] = robust_estimate_Zwiers2010_slicew_24092012(DiffVecORIG,lSWvol,Asym0,AMSK,bvalue,maxk)
...
%     prior   = exp(DM'*mDM);
    ...
    for zpos = 1:vsz(3)
        ...
        while (rms_dDT > thr_DTvar && iter < 10)
            ...
            if(iter==1)
                [Asym_tmp0,w0]   = roustestimate_DT(Asym0slice,AMSKslice,DM,lSWslice,thr_w0,bvalue,sigma,MSKslice2,maxk);
            else
                Asym_tmp0    = Asym_tmp;
            end
            [Asym_tmp,w0] = roustestimate_DT(Asym_tmp0,AMSKslice,DM,lSWslice,thr_w0,bvalue,sigma,MSKslice2,maxk);
...
% ------------------------------------------------------------------------
function [Asym_w,w0] = roustestimate_DT(Asym0,AMSK,DM,lSWvol,thr_w0,bvalue,sigma,MSKslice2,maxk)
...
for i=1:size(res,2)
    MSKtmp  = find(~isnan(res(:,i)));
    MSKtmp2 = AMSK>0;
    tmp(MSKtmp2(MSKtmp),i) = res(MSKtmp,i);
end

if(isempty(MSKslice2) || isempty(find(~isnan(tmp))) || isempty(find(tmp~=0)) )
