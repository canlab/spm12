function [Dconstr,Kconstr,Asym_olsq,Wtensorconstr,lS0]=estimate_Kurtosis(DiffVecORIG,bvalues,binfo,V,MSK1,AS0,VS0,X4,X2,dummy_adaptive,DRad,Kconstrin)
...
D1              = bsxfun(@minus,lS0, D1)/b1;
if(numel(b2)==1)
    D2          = bsxfun(@minus,lS0, D2)/b2; 
    D           = (b2*D1-b1*D2)/(b2-b1);
    K           = 6*(D1-D2)./((b2-b1)*D.^2);
    Dconstr     = constrain_Kurtosis(D,D1,K,D2,b1,b2,1);
else
    D2          = bsxfun(@rdivide,bsxfun(@minus,lS0,D2),b2(binfo.binx));
