function xc = OptimizationProblem(V,bval,B,AMSK,Kmin,Dapp,const)
...
if nargin==0,
    ...
    OptimizationProblem(V,bval,B,AMSK);
    return;
else
...
A       = cat(2,CC1,CC2,CC3); 
A       = A';
b       = cat(2,zeros(1,N), Kmin*Dapp.^2, zeros(1,N));
b       = b'; 
...
if ~exist('x0','var')
    ...
    tmp     = XX*B;
    x0 = (XX*XX')\tmp;
    % hier vernünftige konstrains definieren! 
    x0(find(x0(1,:))<=0)=2/bmax;
    x0(find(x0(2,:))<=0)=1/bmax;
    x0(find(x0(3,:))<=0)=1.1/bmax;
end
...
for i = 1:numel(MSK)
    %%   LArs
%     [xc(:,MSK(i)),iter] = ActiveSetMethod(G,A,c(:,MSK(i)),b,'x0',x0(:,MSK(i)),'tolP', 1e-14,'maxIter', 2000);
%     %% matlab
    [xc(:,MSK(i)),fval,exitflag,output,lambda] = quadprog(G,c(:,MSK(i)),A,b,[],[],[],[],x0(:,MSK(i)),opts);
