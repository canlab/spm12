function xc = OptimizationProblem_single(V,bval,B,AMSK,dummy_opt,Kmin,Dapp,const)
...
%% start defining designe matrix and constrains
szb = size(bval);
MSK = find(AMSK>0); 
if(szb(1)==1)
...
%% Estimating OLS tensor 2nd and 4th order
if (~exist('x0','var') || ~exist('Dapp','var'))
    X2d     = -XX2(:,bMSK); 
    % tmp    = X2d*B;
%    d0     = (X2d*X2d')\tmp;
    d0      = X2d'\B;
...
if ~exist('x0','var')
    ...
    MSKtmpp = find(aa<=0);
    if(~isempty(MSKtmpp))
        x0(:,MSKtmp)    = zeros([size(XX,1) numel(MSKtmp)]);
        x0(1:6,MSKtmp)  = d0(:,MSKtmp);
        ytmp            = A*x0(:,MSKtmp);
        [aa,unused]     = max(ytmp,[],1);
        MSKtmp1         = find(aa>0);
        MSKtmpp1        = find(aa<=0);
        if(~isempty(MSKtmpp1))
            x0(1,MSKtmp(MSKtmp1))=2/bmax;
            x0(2,MSKtmp(MSKtmp1))=1/bmax;
            x0(3,MSKtmp(MSKtmp1))=1.1/bmax;
            for i=4:size(x0,1)
        %             x0(i,MSKtmp)=x0(i,MSKtmpp(1));
                x0(i,MSKtmp(MSKtmp1))=0;
            end
        end 
    end
end
...
xctmp   = cell(numel(MSK),1);
x0tmp   = cell(numel(MSK),1);
btmp   = cell(numel(MSK),1);
ctmp   = cell(numel(MSK),1);
for i=1:numel(MSK)
    x0tmp{i}=x0(:,MSK(i));
    btmp{i}=b(:,MSK(i));
    ctmp{i}=c(:,MSK(i));
...
%   LArs
    if(dummy_opt)
     [xctmp{i},iter] = ActiveSetMethod_superlars(G,-A,ctmp{i},btmp{i},'x0',x0tmp{i},'tolP', 1e-14,'maxIter', 2000);
%     xL = xc(:,MSK(i));
%     %% matlab
    else
        [xctmp{i},fval,exitflag,output,lambda] = ...
        quadprog(G,ctmp{i},A,btmp{i},[],[],[],[],x0tmp{i},opts);
    end
